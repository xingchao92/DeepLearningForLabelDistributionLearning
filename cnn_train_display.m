function [net, info] = cnn_train_display(net, imdb, getBatch, varargin)
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    distributions = gather(res(end).x);
    distributions = reshape(distributions,size(distributions,3),size(distributions,4));
    for i=1:size(distributions,4)
        truth = zeros(size(distributions,1),1);
        truth(labels(i),1) = 1;
        drawdistribution(truth,distributions(:,i));
