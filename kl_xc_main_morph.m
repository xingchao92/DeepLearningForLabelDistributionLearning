function kl_xc_main_morph(varargin)
...
net = kl_xc_net_init('model', opts.modelType, ...
...
% -------------------------------------------------------------------------
function [im,labels,distributions] = getSimpleNNBatch(opts, imdb, batch)
% -------------------------------------------------------------------------
...
if ~isVal
  % training
  im = kl_xc_get_batch(images, opts, ...
                              'prefetch', nargout == 0) ;
else
...
if nargout > 0
  ...
  load Distribution;
  distributions = zeros(size(F,1),size(labels,2));
  distributions = F(labels,:);
end
