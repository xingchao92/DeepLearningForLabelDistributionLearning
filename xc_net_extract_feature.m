function info = xc_net_evaluate(varargin)
...
opts.expDir = fullfile('model', 'morph_50000') ;
opts.imdbPath = fullfile(opts.expDir, 'morph_50000_imdb.mat');
opts.modelPath = fullfile(opts.expDir, 'morph_net.mat') ;
opts.networkType = [] ;
opts.lite = false ;
...
opts.train.errorFunction = 'mae' ;
opts.train.extractFeature = true;
...
if exist(opts.imdbPath)
  ...
  opts.train.truth = imdb.meta.classes;
else
  %
end
...
[net, info] = trainfn(net, imdb, getBatchFn(opts, net.meta), ...
                      opts.train, ...
                      'train', NaN, ...
                      'val', [find(imdb.images.set==1),find(imdb.images.set==2)]);
