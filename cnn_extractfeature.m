function [net, info] = cnn_extractfeature(net, imdb, getBatch, varargin)
...
opts.truth = [];
opts.extractFeature = false;
opts = vl_argparse(opts, varargin) ;


if opts.extractFeature == true
	info.extractFeature = [];
end
...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    ...
    case 'mae'
      opts.errorFunction = @error_mae ;   
      if isempty(opts.errorLabels), opts.errorLabels = {'mae'} ; end
    otherwise
...
for epoch=start+1:opts.numEpochs
  ...
  if numGpus <= 1
    ...
    [~,stats.val,~,info.extractFeature,info.extractClass] = process_epoch(opts, getBatch, epoch, val, 0, imdb, net) ;
    if opts.profile
      profile('viewer') ;
      keyboard ;
    end
...
% -------------------------------------------------------------------------
function err = error_mae(opts, labels, res)
% -------------------------------------------------------------------------
scores = squeeze(gather(res(end-1).x)) ;
[bestScore, best] = max(scores) ;
err = sum(abs(opts.truth(best)-opts.truth(labels)));
...
% -------------------------------------------------------------------------
function  [net_cpu,stats,prof,extractFeature,extractClass] = process_epoch(opts, getBatch, epoch, subset, learningRate, imdb, net_cpu)
% -------------------------------------------------------------------------
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
	...
	if opts.extractFeature == true
        extractFeature(:,batch) = gather(res(end-2).x(1,1,:,:));
        extractClass(1,batch) = labels;
	end
