function net = xc_net_init(varargin)
...
net.layers{end+1} = struct('type', 'rankloss', 'name', 'loss') ;
...
net = add_block(net, opts, '8', 1, 1, 2048, 60, 1, 0) ;
net.layers(end) = [] ;
...
net = add_block(net, opts, '8', 1, 1, 4096, 60, 1, 0) ;
net.layers(end) = [] ;
if opts.batchNormalization, net.layers(end) = [] ; end
