...
for i=1:n
  ...
  switch l.type
    ...
    case 'rankloss'
      res(i+1).x = vl_rankloss(res(i).x, l.class) ; 
      
	case 'klloss'
      res(i+1).x = vl_klloss(res(i).x, l.distributions) ;   
  ...
  forget = forget & ~(strcmp(l.type, 'loss') || strcmp(l.type, 'softmaxloss') || strcmp(l.type, 'rankloss') || strcmp(l.type, 'klloss')) ;
...
if doder
  ...
  for i=n:-1:max(1, n-opts.backPropDepth+1)
    ...
    switch l.type
      ...
      case 'klloss'
        res(i).dzdx = vl_klloss(res(i).x, l.distributions, res(i+1).dzdx) ;  
        
      case 'rankloss'
        res(i).dzdx = vl_rankloss(res(i).x, l.class, res(i+1).dzdx) ;        
